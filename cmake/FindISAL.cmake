...
Find the ISA-L includes and library.

#]=======================================================================]

# Based on the ZLIB module (BSD-3-Clause).
...
if(NOT ISAL_LIBRARY)
  ...
  foreach(search ${_ISAL_SEARCHES})
    find_library(ISAL_LIBRARY_RELEASE
      NAMES ${ISAL_NAMES}
      NAMES_PER_DIR ${${search}}
      PATH_SUFFIXES lib
    )
    find_library(ISAL_LIBRARY_DEBUG
      NAMES ${ISAL_NAMES_DEBUG}
      NAMES_PER_DIR ${${search}}
      PATH_SUFFIXES lib
    )
  endforeach()
...
if(ISAL_INCLUDE_DIR AND EXISTS "${ISAL_INCLUDE_DIR}/isa-l.h")
  file(STRINGS "${ISAL_INCLUDE_DIR}/isa-l.h"
    ISAL_MAJOR_LIST REGEX "^#define ISAL_MAJOR_VERSION"
  )
  ...
  file(STRINGS "${ISAL_INCLUDE_DIR}/isa-l.h"
    ISAL_MINOR_LIST REGEX "^#define ISAL_MINOR_VERSION"
  )
  ...
  file(STRINGS "${ISAL_INCLUDE_DIR}/isa-l.h"
    ISAL_PATCH_LIST REGEX "^#define ISAL_PATCH_VERSION"
  )
  ...
  set(ISAL_VERSION
    "${ISAL_VERSION_MAJOR}.${ISAL_VERSION_MINOR}.${ISAL_VERSION_PATCH}"
  )
endif()

include(FindPackageHandleStandardArgs)
find_package_handle_standard_args(ISAL REQUIRED_VARS
  ISAL_LIBRARY
  ISAL_INCLUDE_DIR
  VERSION_VAR
  ISAL_VERSION
