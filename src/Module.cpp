...
void
ModulePerSequenceQualityScores::write_module(std::ostream &os) {
  os << "#Quality\tCount\n";
  for (std::size_t i = 0; i < FastqStats::kNumQualityValues; ++i) {
...
const std::string ModulePerBaseSequenceContent::module_name =
  "Per base sequence content";
ModulePerBaseSequenceContent::ModulePerBaseSequenceContent(
  const FalcoConfig &config) :
...
ModulePerBaseSequenceContent::summarize_module(FastqStats &stats) {
  ...
  a_pct = std::vector<double>(num_groups, 0.0);
  c_pct = std::vector<double>(num_groups, 0.0);
  g_pct = std::vector<double>(num_groups, 0.0);
  t_pct = std::vector<double>(num_groups, 0.0);
  ...
  for (std::size_t group = 0; group < num_groups; ++group) {
    ...
    for (std::size_t i = base_groups[group].start; i <= base_groups[group].end;
      ...
      if (i < FastqStats::SHORT_READ_THRESHOLD) {
        ...
        a_pos = stats.long_base_count[((i - FastqStats::SHORT_READ_THRESHOLD)
                                       << FastqStats::kBitShiftNucleotide)];
        c_pos = stats.long_base_count[((i - FastqStats::SHORT_READ_THRESHOLD)
                                       << FastqStats::kBitShiftNucleotide) |
                                      1];
        t_pos = stats.long_base_count[((i - FastqStats::SHORT_READ_THRESHOLD)
                                       << FastqStats::kBitShiftNucleotide) |
                                      2];
        g_pos = stats.long_base_count[((i - FastqStats::SHORT_READ_THRESHOLD)
                                       << FastqStats::kBitShiftNucleotide) |
                                      3];
        n_pos = stats.long_n_base_count[i - FastqStats::SHORT_READ_THRESHOLD];
      }
      a_group += a_pos;
      c_group += c_pos;
      g_group += g_pos;
      ...
      a_pos =
        100.0 * a_pos / std::max(std::numeric_limits<double>::min(), total_pos);
      c_pos =
        100.0 * c_pos / std::max(std::numeric_limits<double>::min(), total_pos);
      g_pos =
        100.0 * g_pos / std::max(std::numeric_limits<double>::min(), total_pos);
      t_pos =
    ...
    total =
      static_cast<double>(a_group + c_group + t_group + g_group + n_group);
    a_pct[group] =
      100.0 * a_group / std::max(std::numeric_limits<double>::min(), total);
    c_pct[group] =
      100.0 * c_group / std::max(std::numeric_limits<double>::min(), total);
    g_pct[group] =
      100.0 * g_group / std::max(std::numeric_limits<double>::min(), total);
    t_pct[group] =
      100.0 * t_group / std::max(std::numeric_limits<double>::min(), total);
  }
}
...
void
ModulePerBaseSequenceContent::write_module(std::ostream &os) {
  os << "#Base\tG\tA\tT\tC";
  if (is_bisulfite)
    os << "\tC+T\tA+G";
  ...
  os << "\n";
  for (std::size_t i = 0; i < num_groups; ++i) {
    os << base_groups[i] << "\t" << g_pct[i] << "\t" << a_pct[i] << "\t"
       << t_pct[i] << "\t" << c_pct[i];

    if (is_bisulfite) {
      os << "\t" << c_pct[i] + t_pct[i] << "\t" << a_pct[i] + g_pct[i];
    }
    os << "\n";
  }
}
...
ModulePerBaseSequenceContent::make_html_data() {
  ...
  if (is_bisulfite) {
    for (std::size_t line = 0; line <= 1; ++line) {
      data << "{";
      data << "x : [";
      for (std::size_t i = 0; i < num_groups; ++i) {
        if (base_groups[i].start == base_groups[i].end)
...
ModulePerSequenceGCContent::summarize_module(FastqStats &stats) {
  ...
  gc_deviation = sum_deviation_from_normal(gc_count, theoretical_gc_count);
}
...
void
ModulePerSequenceGCContent::write_module(std::ostream &os) {
  os << "#GC Content\tCount\n";
  for (std::size_t i = 0; i <= 100; ++i)
    os << i << "\t" << gc_count[i] << "\n";
}
...
/******************* PER BASE N CONTENT **********************/
const std::string ModulePerBaseNContent::module_name = "Per base N content";
ModulePerBaseNContent::ModulePerBaseNContent(const FalcoConfig &config) :
...
ModulePerBaseNContent::summarize_module(FastqStats &stats) {
  ...
  n_pct = std::vector<double>(num_groups, 0.0);
  std::size_t this_n_cnt = 0;
  std::size_t this_n_total = 0;
  double this_n_pct = 0.0;

  max_n_pct = 0.0;
  for (std::size_t group = 0; group < num_groups; ++group) {
    std::size_t group_n_cnt = 0, group_n_total = 0;
    for (std::size_t i = base_groups[group].start; i <= base_groups[group].end;
         ++i) {
      this_n_cnt =
        (i < FastqStats::SHORT_READ_THRESHOLD)
          ? (stats.n_base_count[i])
          : (stats.long_n_base_count[i - FastqStats::SHORT_READ_THRESHOLD]);

      this_n_total = (i < FastqStats::SHORT_READ_THRESHOLD)
                       ? (stats.cumulative_read_length_freq[i])
                       : (stats.long_cumulative_read_length_freq
                            [i - FastqStats::SHORT_READ_THRESHOLD]);
      this_n_pct = this_n_cnt / std::max(std::numeric_limits<double>::min(),
                                         static_cast<double>(this_n_total));
      max_n_pct = std::max(max_n_pct, this_n_pct);
      group_n_cnt += this_n_cnt;
      group_n_total += this_n_total;
    }
    n_pct[group] = 100.0 * group_n_cnt /
...
void
ModulePerBaseNContent::write_module(std::ostream &os) {
  os << "#Base\tN-Count\n";
  for (std::size_t i = 0; i < num_groups; ++i) {
...
const std::string ModuleSequenceLengthDistribution::module_name =
  "Sequence Length Distribution";
ModuleSequenceLengthDistribution::ModuleSequenceLengthDistribution(
  const FalcoConfig &config) :
...
ModuleSequenceLengthDistribution::summarize_module(FastqStats &stats) {
  ...
  sequence_lengths = std::vector<std::size_t>(max_read_length, 0);
  ...
  for (std::size_t i = 0; i < max_read_length; ++i) {
    if (i < FastqStats::SHORT_READ_THRESHOLD) {
      ...
      sequence_lengths[i] =
        stats.long_read_length_freq[i - FastqStats::SHORT_READ_THRESHOLD];
    }
...
void
ModuleSequenceLengthDistribution::write_module(std::ostream &os) {
  os << "#Length\tCount\n";
  if (empty_reads > 0)
    os << "0\t" << empty_reads << ".0\n";
  for (std::size_t i = 0; i < max_read_length; ++i) {
...
/************** DUPLICATE SEQUENCES **************************/
const std::string ModuleSequenceDuplicationLevels::module_name =
  "Sequence Duplication Levels";
ModuleSequenceDuplicationLevels::ModuleSequenceDuplicationLevels(
  const FalcoConfig &config) :
...
ModuleSequenceDuplicationLevels::summarize_module(FastqStats &stats) {
  ...
  for (auto v : counts_by_freq) {
    counts_by_freq[v.first] = get_corrected_count(
      stats.count_at_limit, stats.num_reads, v.first, v.second);
  }

  // Group in blocks similarly to fastqc
  for (auto v : counts_by_freq) {
    std::size_t dup_slot = v.first - 1;
    if (v.first >= 10000)
      dup_slot = 15;
    else if (v.first >= 5000)
      dup_slot = 14;
    else if (v.first >= 1000)
      dup_slot = 13;
    else if (v.first >= 500)
      dup_slot = 12;
    else if (v.first >= 100)
  ...
  for (auto &v : percentage_deduplicated)
    v = 100.0 * v /
        std::max(1.0, seq_dedup);  // Percentage of unique sequences in bin
...
void
ModuleSequenceDuplicationLevels::write_module(std::ostream &os) {
  os << "#Total Deduplicated Percentage\t" << total_deduplicated_pct << "\n";
...
const std::string ModuleOverrepresentedSequences::module_name =
  "Overrepresented sequences";
ModuleOverrepresentedSequences::ModuleOverrepresentedSequences(
  const FalcoConfig &config) :
...
std::size_t
get_overlap(const std::string &left, const std::string &right) {
  ...
  for (std::size_t start = 0; start < sz; ++start) {
    std::size_t cur = 0;
...
std::string
ModuleOverrepresentedSequences::get_matching_contaminant(
...
void
ModuleOverrepresentedSequences::write_module(std::ostream &os) {
  if (std::size(overrep_sequences) > 0)
...
/************** ADAPTER CONTENT ***********/
const std::string ModuleAdapterContent::module_name = "Adapter Content";
ModuleAdapterContent::ModuleAdapterContent(const FalcoConfig &config) :
  ...
  if (std::size(adapter_names) != std::size(adapter_seqs))
    throw std::runtime_error("Adapter name and sequence vector sizes differ");

  if (std::size(adapter_names) != std::size(adapter_hashes))
    throw std::runtime_error("Adapter name and hash vector sizes differ");

  num_adapters = std::size(adapter_names);
...
ModuleAdapterContent::summarize_module(FastqStats &stats) {
  num_bases =
    std::max(std::min(stats.max_read_length, FastqStats::SHORT_READ_THRESHOLD),
             ((shortest_adapter_size >= 1) ? (shortest_adapter_size - 1) : 0));

  if (num_bases + 1 >=
      shortest_adapter_size) {  // otherwise this module does not make sense
    adapter_pos_pct.clear();
    for (std::size_t i = 0; i < num_adapters; ++i)
      adapter_pos_pct.push_back(
        std::vector<double>(num_bases - shortest_adapter_size + 1, 0.0));

    std::size_t cnt = 0;
    for (std::size_t i = 0; i < std::size(adapter_pos_pct); ++i) {
      for (std::size_t j = 0; j < std::size(adapter_pos_pct[0]); ++j) {
        cnt = 0;
        // check if position even makes sense
        if (j + std::size(adapter_seqs[i]) < num_bases + 1 &&
            j + std::size(adapter_seqs[i]) >= 1)
          cnt = stats.pos_adapter_count[((j + std::size(adapter_seqs[i]) - 1)
                                         << stats.kBitShiftAdapter) |
                                        i];
    ...
    for (std::size_t i = 0; i < std::size(adapter_pos_pct); ++i) {
      for (std::size_t j = 0; j < std::size(adapter_pos_pct[0]); ++j) {
        ...
        adapter_pos_pct[i][j] /= std::max(std::numeric_limits<double>::min(),
...
void
ModuleAdapterContent::write_module(std::ostream &os) {
  // ADS: number of positions with data calculated
  const std::size_t n_pos_calc =
    (adapter_pos_pct.empty() ? 0 : std::size(adapter_pos_pct[0]));
  ...
  for (std::size_t i = 0; i < n_pos_calc; ++i) {
    os << i + 1;
    for (std::size_t j = 0; j < num_adapters; ++j)
      os << "\t" << adapter_pos_pct[j][i];
    os << "\n";
  }
  // ADS: now be sure to print the full read length
  for (std::size_t i = n_pos_calc; i < num_bases; ++i) {
    os << i + 1;
    for (std::size_t j = 0; j < num_adapters; ++j)
...
ModuleAdapterContent::make_html_data() {
  ...
  for (std::size_t i = 0; i < num_adapters; ++i) {
    ...
    data << "x : [";
    for (std::size_t j = 0; j < n_pos_calc; ++j) {
      data << j + 1;
      if (j + 1 < num_bases)
        data << ",";
    }
    for (std::size_t j = n_pos_calc; j < num_bases; ++j) {
      data << j + 1;
      if (j + 1 < num_bases)
        data << ",";
    }
...
/************** KMER CONTENT ******************************/
const std::string ModuleKmerContent::module_name = "Kmer Content";
...
const std::size_t ModuleKmerContent::MIN_OBS_EXP_TO_REPORT;
const std::size_t ModuleKmerContent::MAX_KMERS_TO_REPORT;
const std::size_t ModuleKmerContent::MAX_KMERS_TO_PLOT;

ModuleKmerContent::ModuleKmerContent(const FalcoConfig &config) :
...
ModuleKmerContent::summarize_module(FastqStats &stats) {
  ...
  obs_exp_max = std::vector<double>(num_kmers, 0.0);
  where_obs_exp_is_max = std::vector<std::size_t>(num_kmers, 0);
  total_kmer_counts = std::vector<std::size_t>(num_kmers, 0);
  ...
  for (std::size_t kmer = 0; kmer < num_kmers; ++kmer) {
    for (std::size_t i = kmer_size - 1; i < num_kmer_bases; ++i) {
      observed_count =
        stats.kmer_count[(i << Constants::bit_shift_kmer) | kmer];
      total_kmer_counts[kmer] += observed_count;
    }
    if (total_kmer_counts[kmer] > 0)
      ++num_seen_kmers;
  }
  ...
  for (std::size_t kmer = 0; kmer < num_kmers; ++kmer) {
    for (std::size_t i = kmer_size - 1; i < num_kmer_bases; ++i) {
      ...
      expected_count = pos_kmer_count[i] /
                       std::max(std::numeric_limits<double>::min(), dividend);
      // ADS: below, denom can't be zero if not above?
      obs_exp_ratio =
        (expected_count > 0) ? (observed_count / expected_count) : 0;
    ...
    if (obs_exp_max[kmer] > MIN_OBS_EXP_TO_REPORT) {
      kmers_to_report.push_back(std::make_pair(kmer, obs_exp_max[kmer]));
    }
  }

  std::sort(begin(kmers_to_report), end(kmers_to_report),
...
ModuleKmerContent::make_grade() {
  const std::size_t lim =
    std::min(std::size(kmers_to_report), MAX_KMERS_TO_REPORT);
  grade = "pass";

  // the worst kmer is at the top
  if (lim > 0) {
    const std::size_t kmer = kmers_to_report[0].first;
...
void
ModuleKmerContent::write_module(std::ostream &os) {
  os << "#Sequence" << '\t' << "Count" << '\t' << "PValue" << '\t'
     << "Obs/Exp Max" << '\t' << "Max Obs/Exp Position" << '\n';
  const std::size_t lim =
    std::min(std::size(kmers_to_report), MAX_KMERS_TO_REPORT);
  for (std::size_t i = 0; i < lim; ++i) {
...
ModuleKmerContent::make_html_data() {
  ...
  std::ostringstream data;
  const std::size_t lim =
    std::min(std::size(kmers_to_report), MAX_KMERS_TO_PLOT);

  // get xlim to plot: whatever the largest position with some
  // reported k-mer is
  std::size_t xlim = 0;
  for (std::size_t i = 0; i < lim; ++i)
    xlim = std::max(xlim, where_obs_exp_is_max[kmers_to_report[i].first]);
  xlim += kmer_size;

  for (std::size_t i = 0; i < lim; ++i) {
    const std::size_t kmer = kmers_to_report[i].first;
    const double log_obs_exp =
      std::log(kmers_to_report[i].second) / std::log(2.0);
    if (!seen_first)
      seen_first = true;
    else
      data << ",";
