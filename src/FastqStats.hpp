...
struct GCModel {
  ...
  GCModel(const int read_length) {
    ...
    for (int pos = 0; pos <= read_length; pos++) {
      ...
      if (high_count < 0)
        high_count = 0;
      if (high_count > read_length)
        high_count = read_length;
      if (low_count > read_length)
        low_count = read_length;

      const int low_pct =
        std::round(100.0 * low_count / static_cast<double>(read_length));
      const int high_pct =
        std::round(100.0 * high_count / static_cast<double>(read_length));
    ...
    for (int pos = 0; pos <= read_length; pos++) {
      ...
      int low_pct =
        (int)round((100 * low_count) / static_cast<double>(read_length));

      int high_pct =
        (int)round((100 * high_count) / static_cast<double>(read_length));
...
struct FastqStats {
  ...
  std::array<size_t, kNumNucleotides * SHORT_READ_THRESHOLD>
    base_count;  // ATGC
  ...
  std::array<size_t, kNumQualityValues * SHORT_READ_THRESHOLD>
  ...
  std::array<size_t, Constants::max_adapters * SHORT_READ_THRESHOLD>
    pos_adapter_count;
  ...
  void
  allocate_new_base(const bool ignore_tile);
